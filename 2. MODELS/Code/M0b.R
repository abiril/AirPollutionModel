...
#all <- unique(all %>% select(-c("dist.to.road", "dist.to.maj", "dist.to.min")))
#View(all[duplicated(all %>% select(-c("dist.to.road", "dist.to.maj", "dist.to.min"))),])
...
#INLA Set-Up
model.Covariates_wNDVI <- c("Time", "PM25PCM", "PM25MO_2", "background", "major_dens.scaled",
                      "primary_dens.scaled", "all_dens.scaled", "minor_dens.scaled",
                      "Temp", "pop", "NDVI_New", "AOD_adj2")

cor(sitedata[,c("logPM25", model.Covariates_wNDVI)], use = "pairwise.complete.obs")
cor(all[,model.Covariates_wNDVI], use = "pairwise.complete.obs")
...
stack_est <- inla.stack(data = list(logPM25 = sitedata$logPM25),
                        ...
                        effects = list(s_index, list(list = data.frame(Intercept = 1), 
                                                     sitedata[,model.Covariates_wNDVI])), tag = "est")
...
stack_pred <- inla.stack(data = list(logPM25 = NA), 
                         A = list(pred, 1),
                         effects = list(s_index, list(list = data.frame(Intercept = 1),
                                                      all[,model.Covariates_wNDVI])), tag="pred")
...
f.iid <- logPM25 ~ -1 + Intercept +  PM25PCM + PM25MO_2 + background + NDVI_New 
...
out <- inla(f.iid,
            ...
            #control.fixed = list(mean = list(PM25PCM = 0.25, PM25MO_2 = 0.1),
            #                     prec = list(PM25PCM = 0.01, PM25MO_2 = 0.01)),
            control.compute = list(dic=TRUE, cpo=TRUE, waic=T, 
                                   mlik=T, return.marginals=T, config=T, 
                                   openmp.strategy="default", smtp="taucs")
)

save(out, file = "MODELS/MODEL/Outputs/M0b.RData")

load(file = "MODELS/MODEL/Outputs/M0b.RData")
...
sitedata <- unique(sitedata %>% select(-c("dist.to.road", "dist.to.maj", "dist.to.min")))

sitedata.month <- subset(sitedata, !(`Site Name` == "Bexley - Slade Green FDMS") & Time == 29)
sitedata.dists <- as.matrix(dist(cbind(sitedata.month$Northing, sitedata.month$Easting)))
sitedata.dists.inv <- 1/sitedata.dists
diag(sitedata.dists.inv) <- 0

sitedata.month$pm25.residual <- sitedata.month$logPM25 - sitedata.month$pm25.estimated

Moran.I(sitedata.month$pm25.residual, sitedata.dists.inv)'

# 0.1969428, 0.0005991613
...
f.m1 <- logPM25 ~ -1 + Intercept +  PM25MO_2 + PM25PCM + background + NDVI_New
...
for(j in 1:6){
  ...
  stack_train <- inla.stack(data = list(logPM25 = sitedata.train$logPM25),
                            A = list(train, 1),
                            effects = list(s_index, list(list = data.frame(Intercept = 1), 
                                                         sitedata.train[,model.Covariates_wNDVI])),
                            tag = "train")
  ...
  stack_test <- inla.stack(data = list(logPM25 = NA), 
                           A = list(test, 1),
                           effects = list(s_index, list(list = data.frame(Intercept = 1),
                                                        sitedata.test[,model.Covariates_wNDVI])),
                           tag="test")
...
save(results, file = "MODELS/MODEL/Outputs/M0b_TimeCV.RData")

load(file = "MODELS/MODEL/Outputs/M0b_TimeCV.RData")
...
results <- do.call(rbind, results_list)
save(results, file = "MODELS/MODEL/Outputs/M0b_SpatialBlockCV.RData")
