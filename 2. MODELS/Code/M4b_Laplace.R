...
save(out, file = "MODELS/MODEL/Outputs/M4b_Laplace.RData")
...
f.m1 <- logPM25 ~ -1 + Intercept + background + PM25PCM +  
  f(Time, NDVI_New, model = "ar1", hyper = hyper.ar13) +
  f(spatial.field, model = spde, group = spatial.field.group, control.group = list(model="ar1", hyper = hyper.ar1.rho)) +
  f(spatial.varying.field, model = spde)
...
for(j in 1:6){
  ...
  A_SVC_train <- inla.spde.make.A(mesh = mesh.s, loc = coords.train,
                                  weights = sitedata.train$PM25MO_2)
  ...
  A_SVC_test <- inla.spde.make.A(mesh = mesh.s, loc = coords.test,
                                 weights = sitedata.test$PM25MO_2)
...
save(results, file = "MODELS/MODEL/Outputs/M4b_Laplace_TimeCV.RData")

load(file = "MODELS/MODEL/Outputs/M4b_Laplace_TimeCV.RData")
...
results <- do.call(rbind, results_list)
save(results, file = "MODELS/MODEL/Outputs/M4b_Laplace_SpatialBlockCV.RData")

load(file = "MODELS/MODEL/Outputs/M4b_Laplace_SpatialBlockCV.RData")

#load(file = "MODELS/MODEL/Outputs/M3_New_NDVI_SpatialBlockCV.RData")
