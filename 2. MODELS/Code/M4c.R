...
theta.ini <- c(
  ...
  log(20), # log range for SVC 1
  log(0.03), # log stdev for SVC 1
  log(20), # log range for SVC 2
  log(0.03)) # log stdev for SVC 2
...
A_SVC_est <- inla.spde.make.A(mesh = mesh.s, loc = coordinates.allyear, weights = sitedata$PM25PCM)

svc_index <- inla.spde.make.index("spatial.varying.field", spde$n.spde)

A_SVC_est2 <- inla.spde.make.A(mesh = mesh.s, loc = coordinates.allyear, weights = sitedata$NDVI_New)

svc_index2 <- inla.spde.make.index("spatial.varying.field2", spde$n.spde)

names(sitedata)
stack_est <- inla.stack(data = list(logPM25 = sitedata$logPM25),
                        A = list(A_est, A_SVC_est, A_SVC_est2, 1),
                        effects = list(s_index, svc_index, svc_index2, 
                                       list(list = data.frame(Intercept = 1), 
...
A_SVC_pred <- inla.spde.make.A(mesh = mesh.s, loc = cbind(all$Easting, all$Northing), weights = all$PM25PCM)

A_SVC_pred2 <- inla.spde.make.A(mesh = mesh.s, loc = cbind(all$Easting, all$Northing), weights = all$NDVI_New)

stack_pred <- inla.stack(data = list(logPM25 = NA), 
                         A = list(A_pred, A_SVC_pred,A_SVC_pred2, 1),
                         effects = list(s_index, svc_index, svc_index2, 
                                        list(list = data.frame(Intercept = 1),
...
f.iid <- logPM25 ~ -1 + Intercept + background +  
  ...
  f(spatial.varying.field2, model = spde)
...
save(out, file = "MODELS/MODEL/Outputs/M4c.RData")

load(file = "MODELS/MODEL/Outputs/M4c.RData")
summary(out)
...
for(j in 1:6){
  ...
  A_SVC_train2 <- inla.spde.make.A(mesh = mesh.s, loc = coords.train, weights = sitedata.train$NDVI_New)
  
  svc_index2 <- inla.spde.make.index("spatial.varying.field2", spde$n.spde)
  
  
  stack_train <- inla.stack(data = list(logPM25 = sitedata.train$logPM25),
                            A = list(A_train, A_SVC_train, A_SVC_train2, 1),
                            effects = list(s_index, svc_index, svc_index2,
                                           list(list = data.frame(Intercept = 1), sitedata.train[,model.covariates])),
                            tag = "train")
  ...
  A_SVC_test2 <- inla.spde.make.A(mesh = mesh.s, loc = coords.test, weights = sitedata.test$NDVI_New)
  
  stack_test <- inla.stack(data = list(logPM25 = NA), 
                           A = list(A_test, A_SVC_test, A_SVC_test2, 1),
                           effects = list(s_index, svc_index, svc_index2,
                                          list(list = data.frame(Intercept = 1),
...
save(results, file = "MODELS/MODEL/Outputs/M4c_TimeCV.RData")

load(file = "MODELS/MODEL/Outputs/M4c_TimeCV.RData")
...
results <- do.call(rbind, results_list)
save(results, file = "MODELS/MODEL/Outputs/M4c_SpatialBlockCV.RData")
