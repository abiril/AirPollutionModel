...
A_SVC_est2 <- inla.spde.make.A(mesh = mesh.s, loc = coordinates.allyear, weights = sitedata$PM25MO_2)
...
A_SVC_pred2 <- inla.spde.make.A(mesh = mesh.s, loc = cbind(all$Easting, all$Northing), weights = all$PM25MO_2)
...
save(out, file = "MODELS/MODEL/Outputs/M4d.RData")

load(file = "MODELS/MODEL/Outputs/M4d.RData")
summary(out)
...
for(j in 1:6){
  ...
  A_SVC_train2 <- inla.spde.make.A(mesh = mesh.s, loc = coords.train, weights = sitedata.train$PM25MO_2)
  ...
  A_SVC_test2 <- inla.spde.make.A(mesh = mesh.s, loc = coords.test, weights = sitedata.test$PM25MO_2)
...
save(results, file = "MODELS/MODEL/Outputs/M4d_TimeCV.RData")'

load(file = "MODELS/MODEL/Outputs/M4d_TimeCV.RData")
...
results <- do.call(rbind, results_list)
save(results, file = "MODELS/MODEL/Outputs/M4d_SpatialBlockCV.RData")
