...
f.iid <- logPM25 ~ -1 + Intercept + background + PM25PCM + PM25MO_2 + 
...
save(out, file = "MODELS/FINAL/Outputs/M3b.RData")

load(file = "MODELS/FINAL/Outputs/M3b.RData")
summary(out)
...
f.m1 <- logPM25 ~ -1 + Intercept + background + PM25MO_2 +PM25PCM + 
  f(spatial.field, model = spde, group = spatial.field.group, control.group = list(model="ar1", hyper = hyper.ar1.rho)) +
  f(spatial.varying.field, model = spde)
...
for(j in 1:6){
  ...
  A_SVC_train <- inla.spde.make.A(mesh = mesh.s, loc = coords.train,
                                  weights = sitedata.train$NDVI_New)
  ...
  A_SVC_test <- inla.spde.make.A(mesh = mesh.s, loc = coords.test,
                                 weights = sitedata.test$NDVI_New)
...
save(results, file = "MODELS/FINAL/Outputs/M3b_TimeCV.RData")

load(file = "MODELS/FINAL/Outputs/M3b_TimeCV.RData")
...
results <- do.call(rbind, results_list)
save(results, file = "MODELS/FINAL/Outputs/M3b_SpatialBlockCV.RData")
