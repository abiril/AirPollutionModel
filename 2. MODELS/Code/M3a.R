...
f.iid <- logPM25 ~ -1 + Intercept +  PM25PCM +  background +  
  f(Time, PM25MO_2, model = "ar1", hyper = hyper.ar1.rho) +
...
save(out, file = "MODELS/MODEL/Outputs/M3a.RData")

load(file = "MODELS/MODEL/Outputs/M3a.RData")
summary(out)
...
f.m1 <- logPM25 ~ -1 + Intercept +   PM25PCM + background +  
  f(Time, PM25MO_2, model = "ar1", hyper = hyper.ar1.rho) +
  f(spatial.field, model = spde, group = spatial.field.group, control.group = list(model="ar1", hyper = hyper.ar1.rho))
...
save(results, file = "MODELS/MODEL/Outputs/M3a_TimeCV.RData")

load(file = "MODELS/MODEL/Outputs/M3a_TimeCV.RData")
...
results <- do.call(rbind, results_list)
save(results, file = "MODELS/MODEL/Outputs/M3a_SpatialBlockCV.RData")
