...
#INLA Set-Up
model.covariates <- c("Time", "PM25PCM", "PM25MO_2", "background", "major_dens.scaled", "NDVI_New", "all_dens.scaled")
...
save(out, file = "MODELS/MODEL/Outputs/M1b.RData")

load(file = "MODELS/MODEL/Outputs/M1b.RData")
summary(out)
...
f.m1 <- logPM25 ~ -1 + Intercept +  PM25PCM + PM25MO_2 + background +  NDVI_New +
  f(spatial.field, model = spde, group = spatial.field.group, control.group = list(model="ar1", hyper = hyper.ar1.rho))
...
save(results, file = "MODELS/MODEL/Outputs/M1b_exp_TimeCV.RData")

load(file = "MODELS/MODEL/Outputs/M1b_exp_TimeCV.RData")
...
results <- do.call(rbind, results_list)
save(results, file = "MODELS/MODEL/Outputs/M1b_exp_SpatialBlockCV.RData")

load(file = "MODELS/MODEL/Outputs/M1b_exp_SpatialBlockCV.RData")
